    ...
    <b-card>
      ...
      >
        ...
        >
              ...
              >
                ...
                >
                  ...
                  >
                    <div
                      v-if="
                        field.typeCol.toUpperCase() == 'DATE' || field.key.toUpperCase() == 'ODATE'
                      "
                    ...
                    <div
                      v-else-if="
                        field.key.toUpperCase() == 'ISATTACHFILE' || field.key.toUpperCase() == 'ISLOCK'
                      "
                    ...
                    >
                      <b-form-select
                        :options="[
                          ...
                          { text: $t('npl.button.yes'), value: true},
                          { text: $t('npl.button.no'), value: false },
                        ]"
                        :value="setFilterValues(field.key)"
                        @change="
                          (value) => changeFilterValues(field.key, value, field.typeCol)
                ...
                <template #cell(RegisToDate)="regis">
                  {{ convertDate(regis.item.RegisToDate) }}
                  <!-- <template>{{test.item.RegisToDate }}</template> -->
                </template>
                <template #cell(BankToDate)="bank">
                  {{ convertDate(bank.item.BankToDate) }}
                </template>
                <template #cell(InsureToDate)="insure">
                  {{ convertDate(insure.item.InsureToDate) }}
                </template>
          ...
          <b-tab>
            <span class="title-category">NGHIỆP VỤ: CÁC LOẠI GIẤY TỜ ĐẦU KÉO</span>
            <form-detail
...
import FormDetail from "@/views/app/function/npl/du-lieu-dau-ky/component/FormDetailDataDateVehicle.vue";
...
export default {
  ...
  data() {
    return {
      ...
      notRequired: ["Note", "IsActive", "EntryID", "OID"],
  ...
  computed: {
    ...
    FactorID() {
      ...
      if (path.includes("function") && path.includes("data-date-vehicle")) {
        return "DataBegin";
      }
      return "";
    },
      ...
      const newArr = arr.filter(item => {
        for (const key in objFilter) {
          ...
          if (objFilter[key].typeCol.toUpperCase() === 'DATE' && value) {
            ...
            if (!item[key].toLowerCase().trim().includes(value.toLowerCase().trim())) {
              return false
            }
          }
          ...
          if (objFilter[key].typeCol.toUpperCase() != 'DATE' && !item[key].toString().toLowerCase().trim().includes(value.toString().toLowerCase().trim())) {
            return false
            }
          // if(objFilter[key].typeCol.toUpperCase() != 'DATE' && value){
          //   if (!item[key].toString().toLowerCase().trim().includes(value.toString().toLowerCase().trim())) {
          //   return false
          //   }
          // }
        }
    ...
    handlePrint(obj) {
      switch (obj.PrintType.toUpperCase()) {
        case "DOWNLOAD":
          ...
          break;
        case "CLIENT":
          let formName = "CÁC LOẠI GIẤY TỜ ĐẦU KÉO";
          handling.printExcel(
