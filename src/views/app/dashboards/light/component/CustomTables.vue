<template>
  <div class="light-dynamic-table">
    <template v-if="dataTable && dataTable.length > 0">
      <b-row class="mb-1" v-if="isTotal || isSearch">
        <b-colxx
          :md="colSearch ? 12 - colSearch : 7"
          class="mt-2"
          v-if="isTotal"
        >
          <span>
        ...
        <b-colxx :md="colSearch ? colSearch : 5" v-if="isSearch">
          <b-form-group
      ...
      >
        ...
        >
          ...
          <template v-else-if="objKey.TypeCol.toUpperCase() === 'STRING-TREE'">
            <template
              v-if="
                (objKey.ClName.toUpperCase() === 'REGIONNAME' &&
                  row.item.Key == 1) ||
                (objKey.ClName.toUpperCase() === 'REGIONNAME' &&
                  row.item.Key == 0) ||
                (objKey.ClName.toUpperCase() === 'REGIONNAME' &&
                  row.item.Key == 2) ||
                (objKey.ClName.toUpperCase() === 'NPLREGIONSNAME' &&
                  row.item.Key == 1) ||
                (objKey.ClName.toUpperCase() === 'NPLREGIONSNAME' &&
                  row.item.Key == 0) ||
                (objKey.ClName.toUpperCase() === 'NPLREGIONSNAME' &&
            ...
            >
              <i
                :style="
                  row.item.Key == 1
                    ? 'font-size: 17px; top: 0; left: 25px;'
                    : row.item.Key == 2
                    ? 'font-size: 17px;top: 0; left: 45px;'
                    : 'font-size: 17px'
                "
                :key="key + 'a'"
                v-if="row.item.isParentCollapse"
                class="fad fa-chevron-right"
          ...
          <template v-else-if="objKey.TypeCol.toUpperCase() === 'VALUE-LED'">
            <template
              v-if="
                ...
                row.item[objKey.ClName.split('Description')[0]] === undefined ||
          ...
          <template v-else>
            ...
            <!-- Edit water-way(phao) -->
            <template v-else-if="objKey.custom">
              <template v-if="objKey.custom.toUpperCase() === 'CUSTOM'">
                <button class="button5">{{ row.item[objKey.ClName] }}</button>
              </template>
            </template>

            <!-- Light chiếu sáng dashboard -->
            <template
              v-else-if="
                objKey.ClName.toUpperCase() === 'LAMPMOVED' ||
                objKey.ClName.toUpperCase() === 'LAMPOPEN' ||
                objKey.ClName.toUpperCase() === 'LAMPERROR' ||
                objKey.ClName.toUpperCase() === 'LAMPTOTAL' ||
                objKey.ClName.toUpperCase() === 'LAMPON' ||
                objKey.ClName.toUpperCase() === 'LAMPOFF' ||
                objKey.ClName.toUpperCase() === 'LAMPDISCONNECT' ||
                objKey.ClName.toUpperCase() === 'LAMPLOWPIN'
              "
            >
              <template v-if="row.item[objKey.ClName] > 0">
                <b-badge
                  style="font-size: 88%; background-color: orange !important"
                  pill
                >
                  {{ row.item[objKey.ClName] }} / {{ row.item.LampTotal }}
                </b-badge>
              </template>
              <template v-if="row.item[objKey.ClName] == 0">
                <b-badge
            ...
            <template v-else>
              <template v-if="objKey.TypeCol.toUpperCase() !== 'MUTEDBIT'">
                ...
                <template v-else>
                  <template v-if="objKey.TypeCol.toUpperCase() === 'COLOR'">
                    <i
                      :style="
                        'font-size: 16px;color:' + row.item[objKey.ClName]
                      "
                      class="fas fa-circle"
                    ></i>
                  </template>
...
export default {
  ...
  data() {
    return {
      isTotal: this.pIsTotal ?? true,
      isSearch: this.pIsSearch ?? true,
...
.button5 {
  background-image: linear-gradient(90deg, $colour9, $colour10);
}
.button5:before {
  background-image: linear-gradient(90deg, $colour9, transparent);
}
.button5:after {
  background-image: linear-gradient(90deg, transparent, $colour10);
}
</style>
