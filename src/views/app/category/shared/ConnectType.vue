    ...
    <b-card>
      ...
      >
        ...
        >
          ...
          <b-tab>
            <span class="title-category">QUẢN LÝ: NHÓM KẾT NỐI</span>
            <form-detail
...
import FormConnectType from "./component/FormConnectType.vue";
import FormHistoryConnectType from "./component/FormHistory.vue";
  ...
  getListMenuRight() {
    ...
    handleDbClick(row) {
      const index = this.dataTable.findIndex(
        (item) => item.ConnectTypeID == row.ConnectTypeID
      );

      const item = this.dataTable[index];

      this.getByID(item.ConnectTypeID);
      this.currentTab = 1;
    ...
    rowSelected(items) {
      ...
      if (items != null) {
        var idPowType;
        items.forEach((el) => {
          idPowType = el.ConnectTypeID;
        });
        var count = 0;
        this.dataTable.forEach((element) => {
          count++;
          if (element.ConnectTypeID == idPowType) {
      ...
      this.$bvModal
        .msgBoxConfirm(
          `${this.$t("modal.confirm-delete")} màu ${
            this.dataTable[this.currentIndex].ConnectTypeName
          } ?`,
          {
    ...
    getDataHistory(index) {
      shareAPI
        ...
        .then((val) => {
          ...
          const item = this.dataTable[index];
          let body = {
            ID: item.ConnectTypeID,
          };
          /** */
          shareAPI.shareConnectTypeByID(body).then((val) => {
    ...
    getAttachFile(index) {
      shareAPI.shareConnectTypeList().then((val) => {
        this.dataTable = val.status
          ? handling.convertDataBitToBoolean(val.data)
          : [];
        index = this.currentIndex;
        const item = this.dataTable[index].ConnectTypeID;
        let body = {
          OID: item.toString(),
    ...
    async handleAttachFile(obj) {
      ...
      try {
        const id = this.dataTable[this.currentIndex].ConnectTypeID;
    ...
    handleChangeAction(key) {
      switch (key) {
        ...
        case "delete":
          ...
          this.currentTab = 1;
          this.disableDelete();
          this.handleDelete(this.dataTable[this.currentIndex].ConnectTypeID);
          break;
        case "cancel":
          this.currentMode = "readOnly";
          this.currentTab = 1;
